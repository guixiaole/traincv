...
while True:
    ...
    tmp_img = cv2.rectangle(tmp_img, (mouse_x + LINE_THICKNESS, mouse_y - LINE_THICKNESS),
                            (mouse_x + text_width + margin, mouse_y - text_height - margin), complement_bgr(color), -1)
    tmp_img = cv2.putText(tmp_img, class_name, (mouse_x + margin, mouse_y - margin), font, font_scale, color,
        ...
        # show edges key listener
            ...
            if is_from_video:
                ...
                if len(object_list) > 0:
                    ...
                    # initial frame
                    init_frame = img.copy()
                    label_tracker = LabelTracker('KCF', init_frame,
                                                 next_frame_path_list)  # TODO: replace 'KCF' by 'CSRT'
                    for obj in object_list:
                        class_index = obj[0]
                        color = class_rgb[class_index].tolist()
                        label_tracker.start_tracker(json_file_data, json_file_path, img_path, obj, color,
                                                    annotation_formats)
        # quit key listener
