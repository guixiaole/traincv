...
def detect_smooth(object_frame):
    ...
    :return:
    '''
    frame_count=20#代表如果已经连续识别超过了50帧（1s的时间）的话，就代表这是信号灯。
    flag=0
    i=1
    while i<len (object_frame):
        ...
        if object_frame[i][0]-object_frame[i-1][0]>1:
            if object_frame[i][0]-object_frame[i-1][0]>frame_count:#如果超过特定的帧数的话，怀疑是下一个视频
                if flag>frame_count:
                    flag=0
                else:
                    while flag:
                        object_frame.pop(i-flag)
                        flag-=0
                        i-=(flag+1)
            else:
